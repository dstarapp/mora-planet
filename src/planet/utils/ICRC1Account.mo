...
module {
  ...
  public func toBlob(account : Account) : Blob {
    return Prim.encodeUtf8(toText(account));
  };

  public func fromBlob(data : Blob) : Result.Result<Account, ParseError> {
    switch (Prim.decodeUtf8(data)) {
      case (?text) {
        return fromText(text);
      };
      case (_) {
        return #err(#malformed("not icrc1 format blob"));
      };
    };
  };
